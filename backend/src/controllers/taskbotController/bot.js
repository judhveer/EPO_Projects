...
async function isMIS(ctx) {
  ...
  const me = await db.Doer.findOne({ where: { telegramId, isApproved: true } });
...
async function showTasksByStatus(ctx, status) {
  ...
  const doer = await db.Doer.findOne({ where: { telegramId } });
  ...
  if (!tasks.length)
    return ctx.reply(`📭 No *${status}* tasks found.`, {
      parse_mode: "Markdown",
    });
...
bot.command("register", async (ctx) => {
  ...
  try {
    let doer = await db.Doer.findOne({ where: { telegramId } });

    if (doer) {
      if (doer.approvalStatus === "PENDING") {
        delete registrationSession[chatId];
        return ctx.reply(
          "🕒 Your registration is pending MIS approval. Please wait or contact MIS"
        );
      }

      if (doer && doer.approvalStatus === "REJECTED") {
        await ctx.reply(
          "❌ Your registration was rejected. Please contact MIS or re-register with correct details."
        );
      }
      ...
      if (doer.department) {
        await ctx.reply(
          `✅ You are already registered as *${doer.name}* in *${doer.department}* department.\n\nDo you want to change your department?`,
          {
            parse_mode: "Markdown",
            ...Markup.inlineKeyboard([
        ...
        showDepartmentOptions(
          ctx,
          "Please select your department to update:",
          "REG_SELECT_DEPT_UPDATE"
        );
      }
    ...
    const firstNameOnly = ctx.from.first_name
      .trim()
      .toUpperCase()
      .split(" ")[0];
    // If no doer exists with this Telegram ID → check by name (maybe unlinked doer exists)
    doer = await db.Doer.findOne({
      where: {
        [Op.or]: [{ name: fullName }, { name: firstNameOnly }],
        telegramId: null,
      },
    });
    ...
    if (doer) {
      ...
      if (doer.department) {
        ...
        await doer.save();
        ctx.reply(
          `✅ ${doer.name}, you are now registered with Telegram ID and department: *${doer.department}*.`,
          {
            parse_mode: "Markdown",
          }
        );
      ...
      ctx.reply(
        `❌ Your name "${fullName}" is not found in the system. Do you want to register yourself?`,
        {
          ...Markup.inlineKeyboard([
...
function showDepartmentOptions(ctx, prompt, prefix = "REG_SELECT_DEPT") {
  ...
  const buttons = departments.map((dep) => [
    Markup.button.callback(dep, `${prefix}_${dep}`),
  ]);
...
bot.action("REG_CHANGE_DEPT", (ctx) => {
  const chatId = getChatId(ctx);
  if (!registrationSession[chatId] || !registrationSession[chatId].doerId)
    return ctx.reply("Session expired. Please use /register again.");
  registrationSession[chatId].step = "choose_department_update";
  showDepartmentOptions(
    ctx,
    "Please select your new department:",
    "REG_SELECT_DEPT_UPDATE"
  );
});
...
bot.action(/REG_SELECT_DEPT_UPDATE_(.+)/, async (ctx) => {
  ...
  try {
    const doer = await db.Doer.findByPk(reg.doerId);
    ...
    ctx.reply(
      `📝 Your request to change department to *${department}* has been sent to MIS for approval. Please wait...`,
      {
        parse_mode: "Markdown",
      }
    );
    ...
    const misDoers = await db.Doer.findAll({
      where: {
        department: "MIS",
    ...
    misDoers.forEach((mis) => {
      if (!mis.telegramId || mis.telegramId.toString().length > 12) {
        console.warn(
          `Skipping MIS user with invalid telegramId:`,
          mis.name,
          mis.telegramId
        );
        return;
        ...
        `📥 *Department Change Request:*\n👤 *${doer.name}*\nOld Dept: *${doer.department}*\nNew Dept: *${department}*`,
        {
          ...
          ...Markup.inlineKeyboard([
            [
              Markup.button.callback(
                "✅ Approve",
                `REG_APPROVE_DEPT_CHANGE_${doer.id}_${department}`
              ),
            ],
            [
              Markup.button.callback(
                "❌ Reject",
                `REG_REJECT_DEPT_CHANGE_${doer.id}`
              ),
            ],
          ]),
        }
...
bot.action(/REG_APPROVE_DEPT_CHANGE_(\d+)_(.+)/, async (ctx) => {
  ...
  try {
    const doer = await db.Doer.findByPk(doerId);
    ...
    const approvedBy = `${ctx.from.first_name} ${
      ctx.from.last_name || ""
    }`.trim();
    ...
    await bot.telegram.sendMessage(
      doer.telegramId,
      `✅ Your department change to *${newDepartment}* has been approved!`,
      {
        parse_mode: "Markdown",
      }
    );
...
bot.action(/REG_REJECT_DEPT_CHANGE_(\d+)/, async (ctx) => {
  ...
  await bot.telegram.sendMessage(
    doer.telegramId,
    `❌ Your request to change department was rejected by MIS.`,
    { parse_mode: "Markdown" }
  );
  ...
  if (
    ...
    await ctx.editMessageText(
      `❌ Department change request for *${doer.name}* was rejected by MIS.`,
      { parse_mode: "Markdown" }
    );
  }
});
...
bot.on("text", async (ctx, next) => {
  ...
  if (reg.step === "ask_name") {
    registrationSession[chatId].pendingName = ctx.message.text
      .trim()
      .toUpperCase();
...
bot.action(/REG_SELECT_DEPT_SELF_ADD_(.+)/, async (ctx) => {
  ...
  if (reg.step !== "ask_department_self_add") {
    console.log(
      "Registration session step mismatch:",
      reg.step,
      "for chat:",
      chatId
    );
  ...
  try {
    ...
    ctx.reply(
      `📝 Your request to register as *${reg.pendingName}* in *${department}* department has been sent for approval. Please wait...`,
      { parse_mode: "Markdown" }
    );
        ...
        `📥 *New Registration Request:*\n👤 Name: *${reg.pendingName}*\n🏢 Department: *${department}*`,
        {
          ...
          ...Markup.inlineKeyboard([
            [
              Markup.button.callback(
                "✅ Approve",
                `REG_APPROVE_REQUEST_${chatId}`
              ),
            ],
            [
              Markup.button.callback(
                "❌ Reject",
                `REG_REJECT_REQUEST_${chatId}`
              ),
            ],
          ]),
        }
...
bot.action(/REG_APPROVE_REQUEST_(\d+)/, async (ctx) => {
  ...
  const doer = await db.Doer.findOne({ where: { telegramId: requesterId } });
  if (!doer) return ctx.reply("Doer not found.");
  // Guard: Prevent double approval
  ...
  await bot.telegram.sendMessage(
    requesterId,
    `✅ Your registration has been approved! You can now use the bot.`,
    { parse_mode: "Markdown" }
  );
...
bot.action(/REG_REJECT_REQUEST_(\d+)/, async (ctx) => {
  ...
  await bot.telegram.sendMessage(
    requesterId,
    `❌ Your registration request was rejected by MIS.`,
    { parse_mode: "Markdown" }
  );
...
bot.action(/REG_SELECT_DEPT_(.+)/, async (ctx) => {
  ...
  try {
    ...
    await doer.save();
    ctx.reply(`✅ You are now registered in *${department}* department!`, {
      parse_mode: "Markdown",
    });
    delete registrationSession[chatId];
  } catch (e) {
...
bot.command("tasks", async (ctx) => {
  ...
  if (!doer.isApproved) {
    return ctx.reply(
      "⛔ You are not approved to use this feature yet. Please wait for MIS to approve your registration."
    );
  }

  if (doer.approvalStatus === "PENDING") {
    return ctx.reply(
      "🕒 You already have a department change request pending approval."
    );
  }

  // Status filter buttons
  return ctx.reply("🔍 *View your tasks by status:*", {
    parse_mode: "Markdown",
    ...Markup.inlineKeyboard([
...
bot.action(/^TASK_DONE_(\d+)$/, async (ctx) => {
  ...
  const doer = await db.Doer.findOne({ where: { telegramId: chatId } });
  ...
  ctx.reply("🎉 *Congrats!* Task marked as *Completed*. 🚀", {
    parse_mode: "Markdown",
  });
...
bot.on("text", async (ctx, next) => {
  ...
  if (session.type === "extension") {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(text)) {
      return ctx.reply(
        "⚠️ *Invalid date format.*\nPlease type the date as YYYY-MM-DD (e.g. 2024-07-15).",
        { parse_mode: "Markdown" }
      );
    }
    ...
    if (
      ...
      return ctx.reply(
        "⚠️ *Invalid or past date.*\nPlease enter a real, future date as YYYY-MM-DD (e.g. 2024-07-15).",
        { parse_mode: "Markdown" }
      );
    }
      ...
      `🚫 *Cancellation Requested*\n\n👤 *Doer:* ${doer.name}\n🆔 *Task ID:* ${task.id}\n📝 *Task:* ${task.task}\n✍️ *Reason:* ${reason}`,
      {
        ...
        reply_markup: Markup.inlineKeyboard([
          [
            Markup.button.callback(
              "✅ Approve Cancel",
              `CANCEL_APPROVE_${task.id}`
            ),
          ],
          [
            Markup.button.callback(
              "❌ Reject Cancel",
              `CANCEL_REJECT_${task.id}`
            ),
          ],
        ]).reply_markup,
      }
...
bot.command("misControl", async (ctx) => {
  if (!(await isMIS(ctx)) && !isBoss(ctx)) {
    return ctx.reply(
      "❌ You are not authorized to access this menu, only MIS and BOSS can access it."
    );
  }
...
bot.command("pendingregistration", async (ctx) => {
  ...
  if (regs.length) {
    await ctx.reply(`🗂️ *Pending Registrations* (${regs.length})`, {
      parse_mode: "Markdown",
    });
    for (const d of regs) {
      await ctx.reply(
        `👤 *${d.name}*\n🏢 Dept: *${d.department || "—"}*\n🆔 Telegram: \`${
          ...
          d.requestedAt ? d.requestedAt.toDateString() : "—"
        }`,
        {
          parse_mode: "Markdown",
          ...Markup.inlineKeyboard([
            [
              Markup.button.callback(
                "✅ Approve",
                `REG_APPROVE_REQUEST_${d.telegramId}`
              ),
            ],
            [
              Markup.button.callback(
                "❌ Reject",
                `REG_REJECT_REQUEST_${d.telegramId}`
              ),
            ],
          ]),
        }
  ...
  if (deps.length) {
    await ctx.reply(`🔁 *Pending Department Changes* (${deps.length})`, {
      parse_mode: "Markdown",
    });
    for (const d of deps) {
        ...
        }\n🕒 Requested: ${d.requestedAt ? d.requestedAt.toDateString() : "—"}`,
        {
          ...
          ...Markup.inlineKeyboard([
            ...
            [
              Markup.button.callback(
                "✅ Approve",
                `REG_APPROVE_DEPT_CHANGE_${d.id}_${newDept}`
              ),
            ],
            [
              Markup.button.callback(
                "❌ Reject",
                `REG_REJECT_DEPT_CHANGE_${d.id}`
              ),
            ],
          ]),
        }
...
bot.command(["remove_employee", "remove"], async (ctx) => {
  ...
  return showDepartmentOptions(
    ctx,
    "Select the department to remove from:",
    "RM_DEPT"
  );
});
...
async function listDeptEmployees(ctx, dept, page = 1) {
  ...
  const { rows, count } = await db.Doer.findAndCountAll({
  ...
  if (!count) {
    return ctx.reply(`📭 No employees found in *${dept}*.`, {
      parse_mode: "Markdown",
    });
  }
  ...
  const buttons = rows.map((d) => [
    Markup.button.callback(
      `👤 ${d.name} (#${d.id})`,
      `RM_PICK_${d.id}_${enc}_${page}`
    ),
  ]);

  const nav = [];
  if (page > 1)
    nav.push(Markup.button.callback("⬅️ Prev", `RM_PAGE_${enc}_${page - 1}`));
  if (page < totalPages)
    nav.push(Markup.button.callback("Next ➡️", `RM_PAGE_${enc}_${page + 1}`));
...
bot.action(/RM_DEPT_(.+)/, async (ctx) => {
  if (!(await isMIS(ctx)) && !isBoss(ctx))
    return ctx.answerCbQuery("Only MIS/Boss.");
  await ctx.answerCbQuery();
...
bot.action(/RM_CONFIRM_(\d+)/, async (ctx) => {
  ...
  // Acknowledge
  return ctx.reply(`🗑️ Removed *${doer.name}* permanently.`, {
    parse_mode: "Markdown",
  });
});
...
bot.command("heybot", async (ctx) => {
  ...
  await ctx.reply("👩‍💼 *EA Control Panel* — Choose an action:", {
    parse_mode: "Markdown",
    ...Markup.inlineKeyboard([
...
bot.action("EA_CANCEL_REQ", async (ctx) => {
  ...
  if (![ROLES.boss, ROLES.ea].includes(chatId)) {
    return ctx.reply(
      "❌ You are not authorized to access cancellation requests."
    );
  }
...
bot.action(/^CANCEL_APPROVE_(\d+)$/, async (ctx) => {
  ...
  if (!task.cancellationRequested)
    return ctx.reply("⚠️ No cancellation requested.");
  ...
  const doer = await db.Doer.findOne({ where: { name: task.doer } });
  if (doer?.telegramId) {
    await bot.telegram.sendMessage(
      doer.telegramId,
...
bot.action(/^EXT_APPROVE_(\d+)$/, async (ctx) => {
  ...
  if (!task) return ctx.reply("❌ Task not found.");
  if (!task.extensionRequestedDate)
    return ctx.reply("⚠️ No extension requested for this task.");
...
const showOptions = (ctx) => {
  ctx.reply(
    "Hi Boss! What would you like to do?",
    Markup.inlineKeyboard([
...
bot.hears(/^(hi|hello|hey)$/i, (ctx) => {
  ...
  if (!isBoss(ctx))
    return ctx.reply(
      "❌ You are not authorized to send this msg only boss can send this msg."
    );
  showOptions(ctx);
});
...
bot.action("ASSIGN", async (ctx) => {
  ...
  const doers = await db.Doer.findAll({ where: { isActive: true } });
  ...
  const buttons = departments.map((dep) => [
    Markup.button.callback(dep, `DEP_${dep}`),
  ]);
...
bot.action(/DEP_(.+)/, async (ctx) => {
  ...
  const buttons = doers.map((d) => [
    Markup.button.callback(d.name, `DOER_${d.id}`),
  ]);
...
bot.on("text", async (ctx, next) => {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(input)) {
    return ctx.reply(
      "⚠️ *Invalid format.*\nPlease type the due date as YYYY-MM-DD (e.g. 2024-07-15).",
      { parse_mode: "Markdown" }
    );
  }
  ...
  if (
    ...
    return ctx.reply(
      "⚠️ *Invalid date.* Please enter a real date as YYYY-MM-DD.",
      { parse_mode: "Markdown" }
    );
  }
  ...
  if (date < today) {
    return ctx.reply(
      "⚠️ *Date is in the past.* Please enter today or a future date.",
      { parse_mode: "Markdown" }
    );
  }
...
bot.action("URGENT", (ctx) => {
  if (!isBoss(ctx))
    return ctx.reply("❌ Only the Boss can perform this action.");
...
bot.action("EDIT", (ctx) => {
  ...
  if (!session || session.step !== "review_task") {
    clearSessions(chatId);
    return ctx.reply(
      "⚠️ Cannot edit right now. Please restart from main menu."
    );
  }
  delete session.task;
  session.step = "waiting_task";
  ctx.reply(
    `Editing task for: ${escapeMarkdown(
      session.doerName
...
bot.action("SEND", async (ctx) => {
  ...
  if (!session || session.step !== "review_task" || !session.task) {
    clearSessions(chatId);
    return ctx.reply(
      "❌ No task to send or not ready for sending. Please restart."
    );
  }
...
Please take appropriate action below if required.`,
    ...
    ctx.reply(
      `⚠️ Could not notify ${session.doerName} — Telegram ID is missing.`
    );
  }
  ...
  ctx.reply(
    `➕ Do you want to assign another task to ${escapeMarkdown(
      session.doerName
    )}?`,
    {
      parse_mode: "Markdown",
...
bot.action("ADD_ANOTHER_YES", (ctx) => {
  ...
  if (!session || session.step !== "add_another_task") {
    clearSessions(chatId);
    return ctx.reply(
      "⚠️ No ongoing task assignment. Please start from the main menu."
    );
  }
...
bot.action("STATUS", async (ctx) => {
  ...
  ctx.reply(
    "📋 Select the task status you want to view:",
    Markup.inlineKeyboard([
...
bot.action("STATUS_PENDING", async (ctx) => {
  ...
  const tasks = await db.Task.findAll({
    ...
    order: [
      [
        sequelize.literal(`CASE WHEN urgency = 'urgent' THEN 0 ELSE 1 END`),
        "ASC",
      ],
      ["createdAt", "DESC"],
    ],
  ...
  const msg = tasks
    .map(
      (t, i) =>
        ...
        `📄 *Description:* ${t.task}\n` +
        `📅 *Due Date:* ${
          t.dueDate ? new Date(t.dueDate).toDateString() : "ASAP"
        }\n`
    )
    .join("\n\n");
