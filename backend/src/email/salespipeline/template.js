export function tplNewResearch({ lead = {}, assigneeName = "", link }) {
  ...
  const estimatedBudget =
    lead.estimatedBudget != null ? esc(String(lead.estimatedBudget)) : "-";
  ...
  const greeting = assigneeName
    ? `Hello ${escHtml(assigneeName)},`
    : "Hello Sales Coordinator,";
  ...
  rows.push({
    label: "Company",
    htmlValue: company || "-",
    textValue: `Company: ${company || "-"}`,
  });
  rows.push({
    label: "Research Date",
    htmlValue: researchDate,
    textValue: `Research Date: ${researchDate}`,
  });
  rows.push({
    label: "Contact",
    htmlValue: contactName || "-",
    textValue: `Contact: ${contactName || "-"}`,
  });
  rows.push({
    label: "Mobile",
    htmlValue: mobile
      ? `<a href="tel:${(mobile || "").replace(
          /[^+\\d]/g,
          ""
        )}" style="color:#0b4a8a;text-decoration:none;font-weight:600;">${escHtml(
          mobile
        )}</a>`
      : "-",
  ...
  rows.push({
    label: "Email",
    htmlValue: email
      ? `<a href="mailto:${email}" style="color:#0b4a8a;text-decoration:none;">${email}</a>`
      : "-",
  ...
  rows.push({
    label: "Region",
    htmlValue: region || "-",
    textValue: `Region: ${region || "-"}`,
  });
  rows.push({
    label: "Est. Budget",
    htmlValue: estimatedBudget,
    textValue: `Est. Budget: ${estimatedBudget}`,
  });
  ...
  if (researchType === "TENDER") {
    ...
    rows.push({
      ...
      htmlValue: tenderOpening,
      textValue: `Tender Opening: ${tenderOpening}`,
    });
    rows.push({
      label: "Tender Closing",
      htmlValue: tenderClosing,
      textValue: `Tender Closing: ${tenderClosing}`,
    });
    rows.push({
      label: "Financial Period",
      htmlValue: escHtml(financialPeriod),
      textValue: `Financial Period: ${financialPeriod}`,
    });
    rows.push({
      label: "Requirements",
      htmlValue: requirements || "-",
      textValue: `Requirements: ${lead.requirements ?? "-"}`,
    });
    rows.push({
      label: "Remarks",
      htmlValue: remarks || "-",
      textValue: `Remarks: ${lead.remarks ?? "-"}`,
    });
  } else {
  ...
  const tableRowsHtml = rows
    .map(
      (r) =>
        `<tr><td style="padding:8px 6px;font-weight:600;width:160px;">${escHtml(
          r.label
        )}</td><td style="padding:8px 6px;">${r.htmlValue}</td></tr>`
    )
    .join("\n");

  const html = `
    <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial; color:#111827; line-height:1.45;">
      <h2 style="margin:0 0 8px;font-size:18px;color:#0b4a8a;">New Research Submitted (${escHtml(
        researchType
      )})</h2>
      ...
      <div style="margin-top:14px;">
        <a href="${escHtml(
          link
        )}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:10px 16px;border-radius:8px;background:linear-gradient(90deg,#0b4a8a,#6c5ce7);color:#fff;text-decoration:none;font-weight:600;">Open Lead & Review</a>
      </div>
...
export function tplAssigned({ lead = {}, assigneeName, roleLabel, link }) {
  ...
  const ticketId = escHtml(lead.ticketId || lead.ticket_id || "");
  const company = escHtml(lead.company || "");
  const approverRemark = escHtml(
    lead.approverRemark || lead.approver_remark || ""
  );
  ...
  const meetingDateTime = fmtDateTime(
    lead.meetingDateTime || lead.meeting_datetime
  );
  const meetingAssignee = escHtml(
    lead.meetingAssignee || lead.meeting_assignee || ""
  );
  const outcomeNotes = escHtml(lead.outcomeNotes || lead.outcome_notes || "");
  const outcomeStatus = escHtml(
    lead.outcomeStatus || lead.outcome_status || ""
  );
  const nextFollowUpOn = fmtDate(lead.nextFollowUpOn || lead.next_follow_up_on);

  const subject = `Action required: ${roleLabel || "Assignee"} assigned for ${
    ticketId || escHtml(lead.ticketId || "")
  }`;
        ...
        <div style="border:1px solid #e6e9ee;border-radius:8px;padding:12px;background:#fff;max-width:600px;">
          <table style="width:100%;border-collapse:collapse;font-size:14px;color:#111827;">
            <tbody>
              ...
              <tr><td style="padding:8px 6px;font-weight:600;">Mobile</td><td style="padding:8px 6px;">${
                mobile
              ...
              <tr><td style="padding:8px 6px;font-weight:600;">Email</td><td style="padding:8px 6px;">${
                email
                  ? `<a href="mailto:${email}" style="color:#0b4a8a;text-decoration:none;">${email}</a>`
                  : "-"
              }</td></tr>
              ...
              ${
                approverRemark
                  ? `<tr><td style="padding:8px 6px;font-weight:600;vertical-align:top;">Approver Remark</td><td style="padding:8px 6px;">${approverRemark}</td></tr>`
                  : ""
              }
            </tbody>
          </table>
        ...
        <div style="margin-top:14px;">
          ...
          ${
            mobile
              ? `&nbsp;&nbsp;<a href="tel:${encodeURIComponent(
                  mobile
                )}" style="display:inline-block;padding:8px 12px;border-radius:8px;border:1px solid #e6e9ee;background:#fff;color:#0b4a8a;text-decoration:none;font-weight:600;">Call Now</a>`
              : ""
          }
        </div>
            ...
            <tbody>
              ...
              <tr><td style="padding:8px 6px;font-weight:600;">Mobile</td><td style="padding:8px 6px;">${
                mobile
                  ? `<a href="tel:${encodeURIComponent(
                      mobile
                    )}" style="color:#0b4a8a;text-decoration:none;">${escHtml(
                      mobile
                    )}</a>`
                  : "-"
              }</td></tr>
              ...
              ${
                isMeetingTypeVisit
                  ? `<tr><td style="padding:8px 6px;font-weight:600;">Location</td><td style="padding:8px 6px;">${
                      location || "-"
                    }</td></tr>`
                  : ""
              }
              <tr><td style="padding:8px 6px;font-weight:600;">Meeting Date & Time</td><td style="padding:8px 6px;">${meetingDateTime}</td></tr>
              <tr><td style="padding:8px 6px;font-weight:600;">Meeting Assignee</td><td style="padding:8px 6px;">${
                meetingAssignee || "-"
              }</td></tr>
              ${
                approverRemark
        ...
        <div style="border:1px solid #e6e9ee;border-radius:8px;padding:12px;background:#fff;max-width:700px;">
          <table style="width:100%;border-collapse:collapse;font-size:14px;color:#111827;">
            <tbody>
              ...
              <tr><td style="padding:8px 6px;font-weight:600;">Outcome Status</td><td style="padding:8px 6px;">${
                outcomeStatus || "-"
              }</td></tr>
              <tr><td style="padding:8px 6px;font-weight:600;">Next Follow-up</td><td style="padding:8px 6px;">${nextFollowUpOn}</td></tr>
              ${
                outcomeNotes
                  ? `<tr><td style="padding:8px 6px;font-weight:600;vertical-align:top;">Outcome Notes</td><td style="padding:8px 6px;">${outcomeNotes}</td></tr>`
                  : ""
              }
            </tbody>
          </table>
