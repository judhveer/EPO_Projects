...
function StatusBadge({ status }) {
  ...
  return (
    <Pill className={map[status] || "bg-gray-100 text-gray-700"}>
...
function UrgencyBadge({ urgency }) {
  ...
  const cls =
    u.includes("high") || u.includes("urgent") || u.includes("critical")
...
export default function TaskDashboard() {
    ...
    async function load() {
      ...
      try {
        ...
        if (!ct.includes("application/json") && typeof data === "string") {
          ...
          throw new Error(
            "Expected JSON but received HTML. Check API route/proxy/auth redirect."
          );
        }
  ...
  const filtered = useMemo(() => {
    return sorted.filter((t) => {
      ...
      if (
        dueFrom &&
        t.dueDate &&
        new Date(t.dueDate) < new Date(`${dueFrom}T00:00:00`)
      )
        return false;
      if (
        dueTo &&
        t.dueDate &&
        new Date(t.dueDate) > new Date(`${dueTo}T23:59:59`)
      )
        return false;
      return true;
    });
  ...
  const pageItems = serverPaging
    ? filtered
    : filtered.slice(startIdx, startIdx + limit);
  const totalPages = Math.max(1, Math.ceil(total / limit));
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Filter by status, urgency, and due date. Export to CSV.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={exportCSV}
              className="px-3 py-2 rounded-lg text-sm font-medium border border-gray-200 bg-white hover:text-blue-700"
            >
              Export CSV
            </button>
          </div>
        ...
        <div className="mt-5 bg-white border border-gray-200 shadow-soft rounded-2xl p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <label className="block text-xs px-3 font-semibold text-gray-500 mb-1">
                Type
              </label>
              <select
                value={urgency}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button
              ...
              className="ml-auto text-lg px-3 rounded-xl border border-gray-300 text-gray-600 hover:text-gray-900 cursor-pointer"
            >
              Reset
            </button>
          </div>
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <select
              ...
              className="rounded-lg px-3 py-[3px] border border-gray-300"
            >
              {[10, 20, 50, 100].map((n) => (
        ...
        {error && (
          <div className="mt-3 p-3 rounded-lg bg-rose-50 text-rose-700 border border-rose-200">
            {error}
          </div>
        )}
          ...
          {view === "table" ? (
                  ...
                  {pageItems.map((t) => (
                    ...
                    >
                      ...
                      <td
                        className={
                          "px-4 py-3 " +
                          (isOverdue(t) ? "text-rose-600 font-semibold" : "")
                        }
                      >
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {t.extensionRequestedDate && (
                            <Pill className="bg-indigo-100 text-indigo-700">
                              Ext: {formatDate(t.extensionRequestedDate)}
                            </Pill>
                          )}
                        </div>
                      </td>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {pageItems.map((t) => (
                <div
                  key={t.id}
                  className="bg-white border border-gray-200 rounded-2xl shadow-soft p-4"
                >
