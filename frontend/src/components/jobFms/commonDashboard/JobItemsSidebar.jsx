...
function Row({ label, value, truncate = false }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2 text-sm">
      <span className="font-medium text-slate-500 shrink-0 min-w-[150px]">{label}:</span>
      <span className={`text-slate-800 ${truncate ? "truncate max-w-xs" : "break-words"}`}
        title={truncate ? String(value) : undefined}>{value}</span>
    </div>
  );
}
...
function Section({ title, children, accent = "slate" }) {
  ...
  return (
    <div className={`border rounded-lg p-3 space-y-2 ${styles[accent] ?? styles.slate}`}>
    ...
    <div className="border rounded-xl p-4 shadow-sm bg-slate-50 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
        <h4 className="font-semibold text-blue-700">
          Item {index + 1}: <span className="text-slate-500 font-normal">{item.category}</span>
        </h4>
        <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded capitalize truncate max-w-[220px]"
          title={item.enquiry_for}>{item.enquiry_for || "—"}</span>
      </div>

      {/* ── Common fields ── */}
      <div className="space-y-1.5">
        <Row label="Size (Finished)" value={item.size} />
        <Row label="Quantity"        value={item.quantity ? `${item.quantity} ${item.uom || ""}`.trim() : null} />
      ...
      {isSingle && (
        <Section title="🧻 Paper Details" accent="blue">
          ...
          <Row label="Paper Type" value={item.selectedPaper?.paper_name || costing?.ssPaper?.paper_name} />
          <Row label="GSM"        value={item.selectedPaper?.gsm        || costing?.ssPaper?.gsm} />
          <Row label="Sheet Size" value={item.selectedPaper?.size_name  || costing?.ssPaper?.size_name} />
          ...
          {costing && (<>
            ...
            <Row label="Sheets + Wastage"  value={costing.ss_sheets_with_wastage} />
            <Row label="Rate / Sheet"      value={costing.ss_sheet_rate ? `₹${costing.ss_sheet_rate}` : null} />
            <div className="mt-2 space-y-1">
              <CostRow label="Sheet Cost"    value={costing.ss_sheet_cost} />
              <CostRow label="Printing Cost" value={costing.ss_printing_cost} />
      ...
      {isMultiple && (
        <>
          ...
          {costing?.ms_inside_costing?.length > 0 ? (
            <div className="space-y-3">
              {costing.ms_inside_costing.map((p, pIdx) => {
                const paperLabel = [p.paper_name, p.gsm ? `${p.gsm} GSM` : null]
                  .filter(Boolean)
                  .join(" · ") || `Paper ${pIdx + 1}`;
                ...
                return (
                  ...
                  >
                    ...
                    <Row label="With Wastage"  value={p.sheets_with_wastage} />
                    <Row label="Rate / Sheet"  value={p.sheet_rate ? `₹${p.sheet_rate}` : null} />
                    ...
                    {p.to_print ? (
                      <>
                        ...
                        <div className="mt-1 space-y-1">
                          ...
                          <CostRow label="Printing Cost" value={p.printing_cost} />
                        </div>
                      </>
                    ) : (
                      <>
            ...
            <Section title="🧻 Inside Paper (legacy)" accent="amber">
              <p className="text-xs text-amber-600 mb-1">
                Saved before multi-paper format. Recalculate to see full breakdown.
              </p>
          ...
          <Section title="📘 Cover Paper" accent="indigo">
            <Row label="Paper Type"  value={item.selectedCoverPaper?.paper_name || costing?.msCoverPaper?.paper_name} />
            <Row label="GSM"         value={item.selectedCoverPaper?.gsm        || costing?.msCoverPaper?.gsm} />
            <Row label="Sheet Size"  value={item.selectedCoverPaper?.size_name  || costing?.msCoverPaper?.size_name} />
            ...
            {costing && (
              <>
                ...
                <Row label="With Wastage" value={costing.ms_cover_sheets_with_wastage} />
                <Row
                  label="Rate / Sheet"
                  value={costing.ms_cover_sheet_rate ? `₹${costing.ms_cover_sheet_rate}` : null}
                />
                <div className="mt-1 space-y-1">
                  <CostRow label="Sheet Cost" value={costing.ms_cover_sheet_cost} />
                  {coverToPrint && (
                    <CostRow label="Printing Cost" value={costing.ms_cover_printing_cost} />
                  )}
                </div>
              </>
            )}
      ...
      {isWide && (
        <Section title="🖼 Wide Format Material" accent="purple">
          <Row label="Material" value={item.selectedWideMaterial?.material_name || costing?.wfMaterial?.material_name} />
          {(item.selectedWideMaterial?.gsm || costing?.wfMaterial?.gsm) && (
            <Row label="GSM" value={item.selectedWideMaterial?.gsm || costing?.wfMaterial?.gsm} />
          )}
          {(item.selectedWideMaterial?.thickness_mm || costing?.wfMaterial?.thickness_mm) && (
            <Row
              label="Thickness"
          ...
          {item.selectedWideMaterial?.roll_width_ft && (
            <Row label="Roll Width" value={`${item.selectedWideMaterial.roll_width_ft} ft`} />
          )}
          {costing && (
            <>
              <Row label="Calc Type"         value={costing.wf_calculation_type?.toUpperCase()} />
              <Row label="Rolls/Boards Used" value={costing.wf_rolls_or_boards_used} />
              ...
              />
              <div className="mt-1 space-y-1">
                <CostRow label="Material Cost" value={costing.wf_material_cost} />
                <CostRow label="Printing Cost" value={costing.wf_printing_cost} />
                <CostRow label="Binding Cost"  value={costing.binding_cost} />
              </div>
            </>
          )}
      ...
      {costing && (
        <Section title="💰 Pricing" accent="green">
          <div className="space-y-1">
            {costing.total_sheet_cost    > 0 && <CostRow label="Total Sheet Cost"    value={costing.total_sheet_cost} />}
            {costing.total_printing_cost > 0 && <CostRow label="Total Printing Cost" value={costing.total_printing_cost} />}
            {costing.binding_cost        > 0 && <CostRow label="Binding Cost"        value={costing.binding_cost} />}
            <CostRow label="Sheet Cost / Copy"    value={costing.sheet_cost_per_copy} />
            <CostRow label="Printing Cost / Copy" value={costing.printing_cost_per_copy} />
            {costing.binding_cost_per_copy > 0 && <CostRow label="Binding / Copy" value={costing.binding_cost_per_copy} />}
        ...
        <Section title="💰 Pricing" accent="green">
          <div className="space-y-1">
            ...
            {costing.binding_cost_per_copy > 0 && (
              <CostRow label="Binding / Copy" value={costing.binding_cost_per_copy} />
            )}
            <div className="border-t border-green-200 pt-1 mt-1">
              <CostRow label="Unit Rate"  value={costing.unit_rate} />
              <CostRow label="Item Total" value={costing.item_total} />
      ...
      {!costing && (item.unit_rate || item.item_total) && (
    ...
    <AnimatePresence>
      <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
        ...
        <div className="shrink-0 bg-blue-600 text-white flex justify-between items-center p-4">
          ...
          <button onClick={onClose} className="text-2xl leading-none hover:opacity-75" aria-label="Close">&times;</button>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          {loading && <p className="text-sm text-gray-500 animate-pulse">Loading items…</p>}
          {!loading && items?.length === 0 && <p className="text-sm text-gray-500">No items found.</p>}
          {!loading && items?.map((item, index) => <ItemCard key={item.id} item={item} index={index} />)}
        </div>
      </motion.div>
    </AnimatePresence>
